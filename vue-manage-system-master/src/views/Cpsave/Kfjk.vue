    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 库房环境监控
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                ...
                <el-button type="primary" @click="addshow">新增巡查记录</el-button>
            ...
            <div>
                <div>
                    ...
                    @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="managerName" label="巡查负责人姓名" align="center"></el-table-column>
                        <el-table-column prop="date" label="巡查日期" align="center"></el-table-column>  
                        <el-table-column prop="venue" label="巡查场馆" align="center"></el-table-column>
                        <el-table-column prop="report" label="巡查报告" align="center"></el-table-column>        
        ...
        <div class="container">
            ...
            <div>
                ...
                :type="alertList.type"
                effect="dark"
                :closable="alertList.closable"
                show-icon
                :class="{'blinking-alert': alertList.isVisible}"
                >
                </el-alert>
                <el-button @click="handleEmail" class="email-btn" :type="alertList.btnType" plain>{{ alertList.context }}</el-button>
            </div>
            <el-divider />
            <div class="video-box">
                <vedio v-for="item in videoList" :videoSource="item.videoSource" :videoTitle="item.videoTitle"></vedio>
            </div>
        </div>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="巡查负责人姓名">
                    <el-input v-model="form.managerName"></el-input>
                </el-form-item> 
                <el-form-item label="巡查场馆">
                    <el-input v-model="form.venue"></el-input>
                </el-form-item> 
        ...
        <el-dialog title="新增藏品" v-model="addVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="巡查场馆">
                    <el-input v-model="addform.venue"></el-input>
                </el-form-item> 
                <el-form-item label="巡查日期">
                    <div class="block">
                        <el-date-picker
                ...
                <el-form-item label="巡查报告">
                    <el-input v-model="addform.report"></el-input>
                </el-form-item> 
            </el-form>
            <template #footer>
                <span class="dialog-footer">
...
import { getSecureRecordAll, getSecureRecordById,addSecureRecord, delSecureRecordById, editSecureRecord } from "../../api/save";
import vedio from "../../components/vedio.vue";
import { hazardWarning } from "../../utils/remaind";
import { whenEmergency } from "../../api/emergency";
...
export default {
    ...
    setup() {
        ...
        const getDataList =async () => {
            await getSecureRecordAll().then(res => {
                // console.log(res);
        ...
        const event = hazardWarning();
        const getEvents = () => {
            // console.log(hazardWarning())
            const res = eventList.find(item => item.event === event)
            if(res.id !== 5) {
                alertList.isVisible = true
                alertList.title = `出现特殊情况-${res.event}!!!`
                alertList.context = '发送紧急邮件'
                alertList.btnType = 'danger'
                alertList.type= 'warning'
                alertList.eventId = res.id
            }else {
                alertList.isVisible = false
                alertList.title = '暂无特殊情况'
                alertList.context = '发送邮件'
                alertList.btnType = 'primary'
        ...
        const handleEmail =async () => {
            const res = eventList.find(item => item.event === event)
            if(res.id !== 5) {
                const res =await whenEmergency(alertList.eventId)
        ...
        const handleSearch = async (key ,text) => {
            if(text === "") {
                ...
                if(key == "1") {
                    try {
                        //藏品id查询
                        const res = await getSecureRecordById(text)
                        // console.log(res);
                        const list = [];
                        list.push(res.data)
        ...
        const handleDelete = (index , row) => {
            ...
            ElMessageBox.confirm("确定要删除吗？", "提示", {
                ...
                .then(async () => {
                    // deleteEmployeeInfo()
                    // console.log(row.id);
                    const res = await delSecureRecordById(parseInt(row.id , 10))
                    // console.log(res);
                    if(res.code == 200) {
        ...
        const saveAdd = async () => {
            // console.log(addform);
            await addSecureRecord(addform.date, addform.venue,addform.managerName, addform.report
            ).then(res => {
                // console.log(123)
                console.log(res)
                if(res.code !== 200) {
        ...
        const saveEdit = async () => {
            ...
            // console.log(form);
            await editSecureRecord(form.id, form.date , form.venue, form.managerName, form.report).then(res => {
                // console.log(addform)
                // console.log(res)
                if(res.code == 200) {
        ...
        //视频监控
        const videoList = ref([
            {id: 0, videoSource: '../assets/video/video.mp4', videoTitle: '陶瓷库'},
            {id: 1, videoSource: '../assets/visdeo/video.mp4', videoTitle: '金银库'},
            {id: 2, videoSource: '../assets/video/video.mp4', videoTitle: '珠宝库'},
        ])
