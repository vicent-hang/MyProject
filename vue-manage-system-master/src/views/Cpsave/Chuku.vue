    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 藏品出库审核
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                ...
                <el-button type="primary" @click="addshow">新增出库记录</el-button>
            ...
            <div>
                <div>
                    ...
                    @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="outPlace" label="借出单位" align="center"></el-table-column>  
                        <el-table-column prop="outTime" label="借出时间" align="center"></el-table-column>
                        <el-table-column prop="returnTime" label="最晚归还时间" align="center"></el-table-column>        
                        <el-table-column prop="outReason" label="借出原因" align="center"></el-table-column>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="借出单位">
                    <el-input v-model="form.outPlace"></el-input>
                </el-form-item> 
                ...
                <el-form-item label="借出原因">
                    <el-input v-model="form.outReason"></el-input>
                </el-form-item> 
            </el-form>
            <template #footer>
        ...
        <el-dialog title="新增藏品" v-model="addVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="借出单位">
                    <el-input v-model="addform.outPlace"></el-input>
                </el-form-item> 
                <el-form-item label="借出时间">
                    <div class="block">
                        <el-date-picker
...
import { getStoreOutAll, getStoreOutById,addStoreOut, delStoreOutById, editStoreOut } from "../../api/collection";
export default {
    name: "yginfo",
    mounted () {
    },
    setup() {
        ...
        const handleSearch = async (key ,text) => {
            if(text === "") {
                ...
                if(key == "1") {
                    try {
                        //藏品id查询
                        const res = await getStoreOutById(text)
                        // console.log(res);
                        const list = [];
                        list.push(res.data)
                        ...
                        console.log(error);
                        ElMessage.error("输入错误，请输入要查找的藏品ID");
                    }
                }else if (key == "2") {
                    try {
                        //分页查询
        ...
        const handleDelete = (index , row) => {
            ...
            ElMessageBox.confirm("确定要删除吗？", "提示", {
                ...
                .then(async () => {
                    // deleteEmployeeInfo()
                    // console.log(row.id);
                    const res = await delStoreOutById(parseInt(row.id , 10))
                    // console.log(res);
                    if(res.code == 200) {
        ...
        const saveAdd = async () => {
            // console.log(addform);
            await addStoreOut(addform.relicId, addform.outPlace,addform.outTime, addform.returnTime, addform.outReason
            ).then(res => {
                // console.log(123)
                console.log(res)
                if(res.code !== 200) {
                    ElMessage.error("增添失败!!! 此藏品不在库中!")
                    return
                }else {
                    //刷新列表
        ...
        const saveEdit = async () => {
            ...
            await editStoreOut(form.id, form.relicId , form.outPlace, form.outTime, form.returnTime, form.outReason).then(res => {
                // console.log(addform)
                // console.log(res)
                if(res.code == 200) {
