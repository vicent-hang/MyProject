    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 藏品入库审核
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                ...
                <el-button type="primary" @click="addshow">新增入库记录</el-button>
...
import { getStoreInAll, getStoreInById,addStoreIn, delStoreInById, editStoreIn } from "../../api/collection";
export default {
    name: "yginfo",
    mounted () {
    },
    setup() {
        ...
        const handleSearch = async (key ,text) => {
            if(text === "") {
                ...
                if(key == "1") {
                    try {
                        //藏品id查询
                        const res = await getStoreInById(text)
                        // console.log(res);
                        const list = [];
                        list.push(res.data)
        ...
        const handleDelete = (index , row) => {
            ...
            ElMessageBox.confirm("确定要删除吗？", "提示", {
                ...
                .then(async () => {
                    // deleteEmployeeInfo()
                    // console.log(row.id);
                    const res = await delStoreInById(parseInt(row.id , 10))
                    // console.log(res);
                    if(res.code == 200) {
        ...
        const saveAdd = async () => {
            // console.log(addform);
            await addStoreIn(addform.relicId, addform.displayTime
            ).then(res => {
                // console.log(123)
                // console.log(res)
                if(res.code !== 200) {
        ...
        const saveEdit = async () => {
            ...
            // console.log(form);
            await editStoreIn(form.id, form.relicId , form.displayTime).then(res => {
                // console.log(addform)
                // console.log(res)
                if(res.code == 200) {
