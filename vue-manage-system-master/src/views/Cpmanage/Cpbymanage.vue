    ...
    <div>
            ...
            <div>
                <div>
                    ...
                    @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="maintContent" label="保养内容"  align="center"></el-table-column>
                        <el-table-column prop="maintTime" label="保养时间" align="center"></el-table-column>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="保养内容">
                    <el-input v-model="form.maintContent"></el-input>
                </el-form-item>
            </el-form>
            <template #footer>
        ...
        <el-dialog title="新增藏品" v-model="addVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="保养内容">
                    <el-input v-model="addform.maintContent"></el-input>
                </el-form-item>
            </el-form>
            <template #footer>
                <span class="dialog-footer">
...
import { getRelicMaintAll, getRelicMaintById,addRelicMaint, delRelicMaintById, editRelicMaint } from "../../api/collection";
export default {
    name: "yginfo",
    mounted () {
    },
    setup() {
        ...
        const getDataList =async () => {
            await getRelicMaintAll().then(res => {
                // console.log(res);
        ...
        const handleSearch = async (key ,text) => {
            if(text === "") {
                ...
                if(key == "1") {
                    try {
                        //藏品id查询
                        const res = await getRelicMaintById(text)
                        console.log(res);
                        const list = [];
                        list.push(res.data)
        ...
        const handleDelete = (index , row) => {
            ...
            ElMessageBox.confirm("确定要删除吗？", "提示", {
                ...
                .then(async () => {
                    // deleteEmployeeInfo()
                    // console.log(row.id);
                    const res = await delRelicMaintById(parseInt(row.id , 10))
                    // console.log(res);
                    if(res.code == 200) {
        ...
        const saveAdd = async () => {
            // console.log(addform);
            await addRelicMaint(addform.relicId,addform.maintTime, addform.maintContent,
        ...
        const saveEdit = async () => {
            ...
            // console.log(form);
            await editRelicMaint(form.id,form.relicId,form.maintTime,form.maintContent,form.leaderName,form.leaderPhone).then(res => {
                // console.log(form)
                console.log(res)
                if(res.code == 200) {
