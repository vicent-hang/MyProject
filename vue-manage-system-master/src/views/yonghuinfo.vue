    ...
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="currentPageData"  border class="table" ref="multipleTable"
                ...
                <el-table-column prop="loginAct" label="登录账号"></el-table-column>
                ...
                <el-table-column prop="expireTime" label="账号过期时间"></el-table-column>
                <el-table-column prop="editTime" label="最近一次编辑时间"></el-table-column>
                <el-table-column prop="lastLoginTime" label="最后登录时间"></el-table-column>
                <el-table-column prop="state" width="55"  label="账号状态"></el-table-column>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="用户角色">
                    <el-select v-model="form.role" placeholder="选择权限" class="handle-select mr10" 
        ...
        <el-dialog title="新增用户" v-model="addVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="用户角色">
                    <el-select v-model="addform.role" placeholder="选择权限" class="handle-select mr10" 
                    @change = "handleRoleChange">
                        <el-option key="1" label="超级管理员" value="1"></el-option>
                        <el-option key="2" label="藏品管理员" value="2"></el-option>
                        <el-option key="3" label="安保管理员" value="3"></el-option>
...
export default {
    ...
    setup() {
        ...
        const getData = async () => {
            await searchuserall().then(res => {
                // console.log(res)
                res.data.forEach(user => {
                    const matchingRole = roleList.find(role => role.id === user.role);
                    if (matchingRole) {
                        user.role = matchingRole.role;
                    }
                });
                res.data.forEach(user => {
                    const matchingRole = stateList.find(item => item.id === user.state);
                    if (matchingRole) {
                        user.state = matchingRole.state;
                    }
                });
                // console.log(res)
        ...
        const handleSearch = async (text) => {
            ...
            if(text === "") {
                ...
                try {
                        await searchuser(text).then(res => {
                            // console.log(res.data);
                            const list = [];
                            list.push(res.data)
                            // console.log(list);
                            const userInfo =  list.map((e) => {
                                return {
                                    id: e.id,
                                    name: e.name,
                                    phone: e.phone,
        ...
        //新增用户时弹窗和保存
        //状态菜单
        let selectionId = reactive({
            authId: '',
            roleId: ''
        })
        const handleAuthChange = (value) => {
            selectionId.authId = value

            console.log("stateId" + selectionId.authId);
        }
        const handleRoleChange = (value) => {
            selectionId.roleId = value
            console.log("roleId" + selectionId.roleId);
        }
        ...
        const saveAdd =async () => {
            ...
            // console.log()
            addform.state = selectionId.authId
        ...
        const handleEdit = (index, row) => {
            ...
            //缺少token
            form.role = selectionId.roleId
            form.state = selectionId.authId
        ...
        const saveEdit = () => {
            ...
            editUser(form.id, form.loginAct, form.loginPwd , form.name, form.phone ,form.email,form.role, form.expireTime , form.state).then(res => {
                // console.log(form.id)
                // console.log(res)
                if(res.code == 200) {
