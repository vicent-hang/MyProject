  ...
  <div class="cart">
    ...
    <div class="cart-box" v-if="isLogin && cartList.length > 0">
      <div class="cart-title">
        <span class="all">共<i>{{ totalCart }}</i>件商品</span>
        <span @click="isEidt=!isEidt" class="edit">
          <van-icon name="edit" />
          编辑
        </span>
      </div>
      ...
      <div class="cart-list">
        <div class="cart-item" v-for="item in cartList" :key="item.id">
          ...
          <div class="show">
            <img :src="item.goods.goods_iamge" alt="">
          </div>
          <div class="info">
            <span class="tit text-ellipsis-2">{{ item.goods.goods_name }}</span>
            <span class="bottom">
              <div class="price">¥ <span>{{ item.goods.goods_price_min }}</span></div>
              <!-- 既希望保留原本参数，又希望通过调用函数传参(传来的参数是value 参数用 item 该表value) => 箭头函数改写 (保持一层)-->
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          ...
          <div v-if="!isEidt" class="goPay" :class="{disabled : selCount === 0}" @click="goPay">结算({{ selCount }})</div>
...
export default {
  ...
  created () {
    if (this.$store.getters.token) {
      this.$store.dispatch('cart/getCarListAction')
    }
  },
  computed: {
    ...mapState('cart', ['cartList']),
    ...mapGetters('cart', ['totalCart', 'selCount', 'selPrice', 'isAllChecked', 'selCheckedItem']),
    isLogin () {
  ...
  methods: {
    ...
    isAllCheck () {
      this.$store.commit('cart/isAllChecked')
    },
    toggleAllCheck () {
    ...
    goPay () {
      ...
      if (this.selCount > 0) {
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCheckedItem.map(item => item.id).join(',')
          }
        })
      }
    }
